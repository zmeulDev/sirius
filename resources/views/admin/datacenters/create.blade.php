@extends('layouts.admin')
@section('content')

<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.datacenter.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.datacenters.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="datacenter_name">{{ trans('cruds.datacenter.fields.datacenter_name') }}</label>
                <input class="form-control {{ $errors->has('datacenter_name') ? 'is-invalid' : '' }}" type="text" name="datacenter_name" id="datacenter_name" value="{{ old('datacenter_name', '') }}" required>
                @if($errors->has('datacenter_name'))
                    <span class="text-danger">{{ $errors->first('datacenter_name') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.datacenter.fields.datacenter_name_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="datacenter_location">{{ trans('cruds.datacenter.fields.datacenter_location') }}</label>
                <input class="form-control {{ $errors->has('datacenter_location') ? 'is-invalid' : '' }}" type="text" name="datacenter_location" id="datacenter_location" value="{{ old('datacenter_location', '') }}" required>
                @if($errors->has('datacenter_location'))
                    <span class="text-danger">{{ $errors->first('datacenter_location') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.datacenter.fields.datacenter_location_helper') }}</span>
            </div>
            <div class="form-group">
                <button class="btn btn-danger" type="submit">
                    {{ trans('global.save') }}
                </button>
            </div>
        </form>
    </div>
</div>



@endsection