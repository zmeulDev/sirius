...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.backupCluster.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.backup-clusters.update", [$backupCluster->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="bck_clus_name">{{ trans('cruds.backupCluster.fields.bck_clus_name') }}</label>
                <input class="form-control {{ $errors->has('bck_clus_name') ? 'is-invalid' : '' }}" type="text" name="bck_clus_name" id="bck_clus_name" value="{{ old('bck_clus_name', $backupCluster->bck_clus_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('bck_clus_type') ? 'is-invalid' : '' }}" name="bck_clus_type" id="bck_clus_type">
                    ...
                    @foreach(App\Models\BackupCluster::BCK_CLUS_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('bck_clus_type', $backupCluster->bck_clus_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
