...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.checklists.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="chk_name">{{ trans('cruds.checklist.fields.chk_name') }}</label>
                <input class="form-control {{ $errors->has('chk_name') ? 'is-invalid' : '' }}" type="text" name="chk_name" id="chk_name" value="{{ old('chk_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('farm') ? 'is-invalid' : '' }}" name="farm_id" id="farm_id" required>
                    @foreach($farms as $id => $entry)
                        <option value="{{ $id }}" {{ old('farm_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('farm'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('backup_cluster') ? 'is-invalid' : '' }}" name="backup_cluster_id" id="backup_cluster_id">
                    @foreach($backup_clusters as $id => $entry)
                        <option value="{{ $id }}" {{ old('backup_cluster_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('backup_tracker') ? 'is-invalid' : '' }}" name="backup_tracker_id" id="backup_tracker_id">
                    @foreach($backup_trackers as $id => $entry)
                        <option value="{{ $id }}" {{ old('backup_tracker_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('domain_user') ? 'is-invalid' : '' }}" type="text" name="domain_user" id="domain_user" value="{{ old('domain_user', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('stlr_cases') ? 'is-invalid' : '' }}" type="text" name="stlr_cases" id="stlr_cases" value="{{ old('stlr_cases', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('checks') ? 'is-invalid' : '' }}" type="text" name="checks" id="checks" value="{{ old('checks', '') }}" required>
