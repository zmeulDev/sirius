...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.farm.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.farms.update", [$farm->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('farm_name') ? 'is-invalid' : '' }}" type="text" name="farm_name" id="farm_name" value="{{ old('farm_name', $farm->farm_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('farm_datacenters') ? 'is-invalid' : '' }}" name="farm_datacenters[]" id="farm_datacenters" multiple required>
                    @foreach($farm_datacenters as $id => $farm_datacenter)
                        <option value="{{ $id }}" {{ (in_array($id, old('farm_datacenters', [])) || $farm->farm_datacenters->contains($id)) ? 'selected' : '' }}>{{ $farm_datacenter }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('farm_prefix') ? 'is-invalid' : '' }}" type="text" name="farm_prefix" id="farm_prefix" value="{{ old('farm_prefix', $farm->farm_prefix) }}" required>
            ...
            <div class="form-group">
                <label class="required" for="farm_domains">{{ trans('cruds.farm.fields.farm_domain') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('farm_domains') ? 'is-invalid' : '' }}" name="farm_domains[]" id="farm_domains" multiple required>
                    @foreach($farm_domains as $id => $farm_domain)
                        <option value="{{ $id }}" {{ (in_array($id, old('farm_domains', [])) || $farm->farm_domains->contains($id)) ? 'selected' : '' }}>{{ $farm_domain }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('farm_bck_trackers') ? 'is-invalid' : '' }}" name="farm_bck_trackers[]" id="farm_bck_trackers" multiple required>
                    @foreach($farm_bck_trackers as $id => $farm_bck_tracker)
                        <option value="{{ $id }}" {{ (in_array($id, old('farm_bck_trackers', [])) || $farm->farm_bck_trackers->contains($id)) ? 'selected' : '' }}>{{ $farm_bck_tracker }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('farm_bck_clusters') ? 'is-invalid' : '' }}" name="farm_bck_clusters[]" id="farm_bck_clusters" multiple required>
                    @foreach($farm_bck_clusters as $id => $farm_bck_cluster)
                        <option value="{{ $id }}" {{ (in_array($id, old('farm_bck_clusters', [])) || $farm->farm_bck_clusters->contains($id)) ? 'selected' : '' }}>{{ $farm_bck_cluster }}</option>
                    @endforeach
                </select>
